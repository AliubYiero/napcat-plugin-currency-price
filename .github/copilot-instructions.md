...
| 模式 | 实现位置 | 说明 |
|------|----------|------|
| 单例状态 | `packages/plugin/src/core/state.ts` | `pluginState` 全局单例，持有 ctx、config 引用 |
...
| 文件 | 职责 |
|------|------|
| `packages/plugin/src/index.ts` | 插件入口，导出生命周期钩子 (`plugin_init`, `plugin_onmessage`, `plugin_onevent`, `plugin_cleanup`) 和配置管理钩子 |
| `packages/plugin/src/config.ts` | 默认配置 `DEFAULT_CONFIG` 和 WebUI 配置 Schema 构建 (`buildConfigSchema`) |

### 核心状态

| 文件 | 职责 |
|------|------|
| `packages/plugin/src/core/state.ts` | 全局状态单例 `pluginState`，管理 ctx 引用、配置持久化、数据文件读写、selfId、定时器、统计信息 |
| `packages/shared/src/index.ts`（由 plugin/webui 重导出） | TypeScript 类型定义（`PluginConfig`, `GroupConfig`, `ApiResponse`） |

### 业务服务

| 文件 | 职责 |
|------|------|
| `packages/plugin/src/services/api-service.ts` | WebUI API 路由注册（状态、配置、群管理接口） |

### 消息处理

| 文件 | 职责 |
|------|------|
| `packages/plugin/src/handlers/message-handler.ts` | 消息事件入口，命令解析、CD 冷却、消息发送工具（含合并转发消息）、权限检查 |

### 前端 WebUI

| 文件 | 职责 |
|------|------|
| `packages/webui/` | React + Vite 前端项目，管理界面用于配置和状态展示 |
...
1. 构建 WebUI 前端（在仓库根目录执行 `pnpm run build`（或仅构建 webui：`pnpm --filter @napcat-plugin-template/webui build`））
...
- **前端图标**：使用 SVG 图标，不要使用 emoji。推荐方式：
  - 将 SVG 封装为 React 组件（参考 `packages/webui/src/components/icons.tsx`）

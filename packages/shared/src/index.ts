...
/**
 * 一个通货的抓取目标: 名字 + 要不要抓它的**详情**。
 *
 * ⚠️ **详情是逐通货开的, 不是游戏级开关**。取详情要为每个通货多做一次点击与一次面板
 * 等待, 抓取时长与失败面都随之变大——应当由用户显式付这个代价。
 *
 * ⚠️ **它与别处"另存一份清单"的写法不同, 是刻意的**: 两个清单(通货清单 + 详情清单)
 * 迟早失步, 表现是"删掉一个通货, 另一个清单里还留着", 而且要删两处。
 */
export interface CurrencyConfig {
    /** 通货名, 与站点名称**精确匹配** */
    name: string;
    /**
     * 是否抓这个通货的详情 (成交量 + 前 N 条挂单)。
     * **默认关闭**——零配置下的默认形态是"有价格、没有盘口", 那已经是一个能用的答案。
     */
    detail: boolean;
}
...
export interface CatalogConfig {
     ...
     * 要抓的通货清单, 与站点名称**精确匹配**。
     *
     * ⚠️ **三份 `currencyList` 彼此不通用**: 同名通货跨游戏独立, 且站点上各专区提供的
     * 通货本就不同——把流放之路2 的清单照抄给流放之路1, 表现是**静默产出 0**(名称匹配
     * 不上, 不报错)。这正是游戏级 `missing` 要兜的那类配置错误。
     */

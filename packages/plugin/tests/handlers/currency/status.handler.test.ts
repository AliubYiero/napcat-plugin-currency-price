...
describe('status — 通知开关与已订阅游戏', () => {
    it('无订阅时如实说没有订任何游戏, 且不列出游戏数据行', async () => {
        expect(await send(groupMessage('#currency status'))).toBe(
            ['#currency 状态', '本群通知：已关闭', '已订阅游戏：（无）'].join('\n'),
        );
    });
    ...
    it('**status 是本会话的**: 别的群订了游戏, 本群的状态里不出现', async () => {
        ...
        expect(await send(groupMessage('#currency status', { groupId: '999' }))).toContain(
            '已订阅游戏：（无）',
        );
    });

    it('私聊说"本会话通知"而不是"本群通知"', async () => {
        expect(await send(privateMessage('#currency status'))).toContain('本会话通知：已关闭');
    });
});
...
describe('status — 浏览器行的可见性', () => {
    it('**只有「私聊 + superAdmin」看得到**浏览器状态行', async () => {
        const reply = await send(
            privateMessage('#currency status', { userId: SUPER_ADMIN, subType: 'friend' }),
        );
    ...
    it('超管在**群里**看不到: 那条信息是给部署者自己看的, 不该发到群里', async () => {
        const reply = await send(
            groupMessage('#currency status', { userId: SUPER_ADMIN, role: 'member' }),
        );
    ...
    it('好友私聊的普通用户看不到', async () => {
        const reply = await send(privateMessage('#currency status', { userId: '10002' }));

        expect(reply).not.toContain('浏览器');
    });

    it('群管理员看不到, 群普通成员也看不到', async () => {
        expect(await send(groupMessage('#currency status', { role: 'admin' }))).not.toContain(
            '浏览器',
        );

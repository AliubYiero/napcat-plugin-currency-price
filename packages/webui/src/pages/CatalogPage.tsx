...
import { IconTerminal, IconSave, IconRefresh, IconX } from '../components/icons'

/** 区服组合在输入框里的分隔符。一行 = 一个区服组合，层数由分隔出的段数决定 */
const ZONE_SEPARATOR = '/'

const zoneRowToText = (row: string[]) => row.join(` ${ZONE_SEPARATOR} `)
const textToZoneRow = (text: string) =>
    text.split(ZONE_SEPARATOR).map((part) => part.trim()).filter((part) => part.length > 0)
...
export default function CatalogPage() {
    const [catalogs, setCatalogs] = useState<CatalogConfig[] | null>(null)
    ...
    const fetchCatalogs = useCallback(async () => {
        try {
            const res = await noAuthFetch<CatalogConfig[]>('/catalogs')
    ...
    const update = (index: number, patch: Partial<CatalogConfig>) => {
        setCatalogs((prev) => prev?.map((item, i) => (i === index ? { ...item, ...patch } : item)) ?? null)
    }

    const addGame = () => {
        setCatalogs((prev) => [
            ...(prev ?? []),
            { name: '', pageUrl: '', zoneConfigs: [['']], currencyList: [] },
        ])
    }

    const removeGame = (index: number) => {
        const target = catalogs?.[index]
        if (!window.confirm(`删除「${target?.name || '未命名游戏'}」的整份配置？`)) return

        setCatalogs((prev) => prev?.filter((_, i) => i !== index) ?? null)
    }
    ...
    const save = async () => {
        ...
        const incomplete = catalogs.filter((c) => !c.name.trim() || !c.pageUrl.trim())
        if (incomplete.length > 0) {
            showToast(`有 ${incomplete.length} 个游戏没填游戏名或页面 URL`, 'warning')
            return
        }
        ...
        return (
            <div className="flex items-center justify-center h-64 empty-state">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="text-gray-400 text-sm">加载分区配置中...</div>
                </div>
            </div>
        )
    }
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 animate-fade-in-down">
                <p className="text-xs text-gray-400">
                    一个游戏 = 一份抓取目标。保存后<strong className="text-gray-500 dark:text-gray-300">下一次抓取即生效</strong>，无需重启。
                </p>
                <div className="flex items-center gap-2">
                    <button className="btn btn-ghost text-xs" onClick={fetchCatalogs}>
                        <IconRefresh size={13} />
                        重新加载
                    </button>
                    <button className="btn btn-ghost text-xs" onClick={addGame}>
            ...
            {catalogs.length === 0 && (
                <div className="card py-12 text-center empty-state">
                    <p className="text-gray-400 text-sm">还没有配置任何游戏</p>
...
function GameCard({ catalog, onChange, onRemove }: GameCardProps) {
    const setZoneRow = (rowIndex: number, row: string[]) => {
        onChange({ zoneConfigs: catalog.zoneConfigs.map((r, i) => (i === rowIndex ? row : r)) })
    }

    const addZoneRow = () => onChange({ zoneConfigs: [...catalog.zoneConfigs, []] })

    const removeZoneRow = (rowIndex: number) =>
        onChange({ zoneConfigs: catalog.zoneConfigs.filter((_, i) => i !== rowIndex) })
    ...
    return (
        <div className="card p-5 hover-lift animate-fade-in-up space-y-5">
            ...
            <div className="flex items-start justify-between gap-4">
                ...
                <button className="btn btn-ghost text-xs text-red-400" onClick={onRemove}>
                    <IconX size={13} />
                    删除
                </button>
            </div>
            ...
            <CommitInput
                label="通货清单"
                desc="要抓的通货名，用英文逗号分隔。与站点名称精确匹配——写错的表现是静默产出 0，不报错"
                value={catalog.currencyList.join(', ')}
                placeholder="神圣石, 崇高石"
                onCommit={(value) =>
                    onChange({
                        currencyList: value
            ...
            <div>
                <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-medium text-gray-800 dark:text-gray-200">区服组合</span>
                    <button className="btn btn-ghost text-xs" onClick={addZoneRow}>
                        添加组合
                    </button>
                </div>
                <div className="text-xs text-gray-400 mb-2">
                    一行一个组合，用 <code className="font-mono">/</code> 分隔层级。层数随游戏而变——
                    流放之路是 3 级（<code className="font-mono">国服 / 赛季 / 普通</code>），
                    火炬之光是 2 级（<code className="font-mono">赛季 / 普通</code>）
                </div>

                {catalog.zoneConfigs.length === 0 ? (
                    <p className="text-xs text-gray-400 py-1">还没有组合，这个游戏不会被抓取</p>
                ) : (
                        ...
                        {catalog.zoneConfigs.map((row, rowIndex) => (
                            <div key={rowIndex} className="flex items-center gap-2">
                                <CommitInput
                                    ...
                                    mono
                                    bare
                                    onCommit={(value) => setZoneRow(rowIndex, textToZoneRow(value))}
                                />
                                <button
                                    className="btn btn-ghost text-xs text-red-400 flex-shrink-0"
                                    onClick={() => removeZoneRow(rowIndex)}
                                >
                                    <IconX size={12} />
                                </button>
                            </div>
 ...
 */
function CommitInput({ label, desc, value, placeholder, mono, bare, onCommit }: {
    label?: string
    desc?: string
    value: string
    placeholder?: string
    ...
    return (
        <div>
            ...
            {desc && <div className="text-xs text-gray-400 mb-2">{desc}</div>}
            {input}
        </div>
    )
}

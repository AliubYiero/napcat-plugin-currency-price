...
| 文档 | 覆盖范围 |
| --- | --- |
| [design.md](./design.md) | **本插件的设计文档**: 目标、配置、数据模型、调度、通知、指令、编码顺序、验收依据 |
| `CONTEXT.md` (根目录) | 领域术语表 (含 _Avoid_ 反用词) 与默认值语义声明 |
| `CLAUDE.md` (根目录) | 插件项目结构、构建运行方式、本项目实例化说明 |
| [adr/](./adr/) | 架构决策记录。`0001` 页面点击而非直连接口 (**不可违背**, 转自抓取源项目)、`0002` 指令失败一律显式反馈、`0003` 内联打包 playwright-core 及随之而来的构建期补丁与能力约束、`0004` 分发层做成纯函数并引入 validateArgs 钩子 |

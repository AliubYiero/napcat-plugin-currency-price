...
3. 【必须】**校验失败反馈不对称是设计而非偶然**: 权限不足与未知指令静默 (防权限探测), 作用域不符回复提示 (可用性)。
   ...
   > ⚠️ **本策略是默认值, 不是绝对律**。项目可在 ADR 中声明不同的反馈策略——判据见 [development-pattern.md](./development-pattern.md) 的"ADR 纪律"。当插件面向熟人小群、且指令面中没有敏感操作时, "防权限探测"的收益趋近于零, 而静默造成的可用性损耗每天发生; 此时应改为显式反馈并在 ADR 中记录理由与如实区分三类失败的文案。参考实现见本项目的 `docs/adr/0002`。
...
【应该】**只有前缀、无参数时路由到 `help`**。`rawMessage` 去掉前缀后为空 (如 `#cmd` 单独出现) 时, 参数数组是 `['']`, 按上述规则会静默返回。若一级命名空间中存在 `help` 指令, 应在此时直接路由到它——"敲个前缀看看有什么"是最自然的用户行为, 用静默回应它是纯粹的可用性损失。

- 【应该】该兜底只在**一级命名空间存在 `help`** 时生效; 没有 `help` 的插件保持静默。
- 【必须】只匹配"**完全没有参数**"这一种形态 (`args.length === 1 && args[0] === ''`), 不扩展到其他情况, 避免把合法的空参数调用吞掉。
...
- [ ] 【应该】若提供 `help` 指令, 确认分发层已实现"只有前缀、无参数时路由到 `help`"的兜底。

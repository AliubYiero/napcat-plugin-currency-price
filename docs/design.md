...
```
「流放之路2」2026-09-21 02:09 实时千岛通货价格

【国服 / 闪回赛季 / 普通】
1. 「神圣石」 8.235 个/元 (0.1214 元/个) · 成交量 530.6w/天
   前5个挂单：
    (1) 2000 个 = 251.00 元 (7.9681 个/元)
    (2) 300   个 = 37.65   元 (7.9681 个/元)
    (3) 1000 个 = 125.50 元 (7.9681 个/元)
    (4) 400   个 = 50.20   元 (7.9681 个/元)
    (5) 330   个 = 41.56   元 (7.94     个/元)
2. 「卡兰德的魔镜」 0.004 个/元 (277.7801 元/个)

【国际服 / 闪回赛季 / 专家】
 · 「初火源质」 826.182 火/元 (0.0012 元/火) · 成交量 52.8w/天
...
- **通货名带 `「」`**, 与头部的游戏名同一种写法——两者都是"某物的名字", 在一个消息里用两种写法会让人以为它们不是一类东西。
...
- **成交量标 `/天`**。站点给的是**日**成交量，不标单位的话 `530.6w` 会被读成累计值。⚠️ **这个「天」是站点语义，不是我们从数字上推出来的**——若站点某天改成累计口径，这一行会把人引到错的理解上（见 §21）。
- **详情块**（[ADR-0005](adr/0005-order-book-capture-and-price-direction.md) 结论 1）:成交量原文跟在价格行末尾；挂单块另起，每行是 `库存 个 = 总价 元 (比率价 个/元)`——即「**这一单**有多少个 = 买下它要多少元（1 元能买几个）」。挂单行与价格行因此**读法相同**:主方向（`个/元`）在括号里，元方向的结论在等号右边。
  - **不写算式**。改造前是 `库存 ÷ 比率价 = 总价`，那个形状把"除法"摆到了台面上；而这一行要回答的是"这一单多少钱、单价多少"，不是"这两个数怎么算出来的"。
  - **三列各自补齐到本块最长的那个数**（库存 / 总价 / 比率价）。最终空格数是 `1 + 2 × (本列最长字符数 − 本行字符数)`：分隔符那一个 + 每短一个字符再补两个。按**字符数**算，不按数字位数。**逐块算**——不同通货的挂单各排各的，跨通货对齐要用到别的通货的数字宽度，与这一块自己的可读性无关。**价格行不参与**（那一行是"这个通货什么价"，不是一张要竖着扫的表）。
    > ⚠️ 这是**可读性上的取舍，不是排版精确性**：按字符数补空格在一个比例字体里并不能真正对齐（中英混排更是如此）。它换来的是"短的那个数后面空得更开"，眼睛更容易分段。
...
- [ ] 通货名带 `「」`, 与头部的游戏名同一种写法
- [ ] 成交量标 `/天`（站点是日口径, 不标会被读成累计值）
- [ ] 挂单块三列按本块最长的数补齐（`1 + 2 × 差几位` 个空格）, 且**逐块算**; 价格行不参与对齐
...
- **成交量的「天」是站点口径，不是我们从数字上推出来的**。消息里写 `530.6w/天`，而数据里存的只是原文 `530.6w`——**没有任何字段记录"这是日口径"**。如果站点某天改成累计值（或某个游戏的口径本来就不同），这一行会把用户引到错的理解上，而数据本身看不出异常。唯一的防线是 §20.1 那条验收：真机上核对一次站点的成交量文案是否明写"日"。
